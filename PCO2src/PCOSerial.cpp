...
void pco::pcoTask()
{
    ...
    if (getIntParam(pco_is_running)>0)
    {
           ...
           //release serial mutex in case some exception caused us not to release properly...
        ...
        //releaseGrabberMutex();
            ...
            callParamCallbacks();
                if ( 
                        (getIntParam(is_com_open)==1) &&
                         (getIntParam(ADAcquire)==1) &&
                         ...
                         )
                     {
                        ...
                        lf.log(mesx);
                        sprintf(mesx,"cor_est_buffers_left %d",getIntParam(cor_est_buffers_left));
                        lf.log(mesx);

                        sprintf(mesx,"total_missed_frames %d",getIntParam(total_missed_frames));
                        lf.log(mesx);
                        sprintf(mesx,"recent_missed_frames %d",getIntParam(recent_missed_frames));
                        lf.log(mesx);

                        sprintf(mesx,"cor_cant_get_ndarray %d",getIntParam(cor_cant_get_ndarray));
                        lf.log(mesx);
        ...
        if ((getIntParam(pco_dump_camera_memory)==1) && (getIntParam(ADAcquire)==0))
        {
...
//                     (time2.isElapsed((double)getIntParam(pco_check_time2)/1000.0))))
                     if ( 
                         ((getIntParam(ADAcquire)==0) && (getIntParam(pco_force_check)==1) ) ||
...
//we must release serial mutex to mess w./ grabber so we only have one mutex at a time
...
void pco::dbgSerial(void)
{
    ...
    if (getIntParam(pco_dbg_serwrite)>0)
    {
        ...
        len=strlen(msg);
        //format is 00ffef23a2, all hex nibbles- take byte at a time and comvert to real bivnary codes
        m=0;
        for (k=0;k<len;k=k+2)
        {
            bytestr[0]=msg[k];
        ...
        #else
        pasynOctetSyncIO->write(
            pasynUserSerial, 
            (const char*)serstr,
            getIntParam(pco_dbg_serwrite),
            0,
            &mynwrite);
        #endif
    }
    else if (getIntParam(pco_dbg_serread)>0)
    {
        ...
        #else
        pasynOctetSyncIO->read(
            pasynUserSerial, 
            (char*)serstr, 
            getIntParam(pco_dbg_serread), 
            0,
            &mynread, 
            &myeomreason);
        #endif
    ...
    int len)
{
    ...
    //    code_hi = (unsigned char)((code&(255*256)) / 256);
        ...
        if (getIntParam(pco_dbg_serprint)==code)
            is_print=true;
        ...
        // need length -2 because getData has no codehi and codelo, but length includes it...
        #ifndef USEASYNSERIAL
        serial_port->write(cmd.getData(), length-2);
        #else
        pasynOctetSyncIO->write(
            pasynUserSerial, 
            (const char*)cmd.getData(),
            length-2,
            0,
            &mynwrite);
        #endif
    ...
    if (length>256)
    {
        ...
        //because openCamera grabs serial mutex, we release it here so we dont grab twite
        //releaseSerialMutex();
        OpenCamera();
        //grabSerialMutex();
        com_error_counter++;
    ...
    if (rsp.getCode() == rsp.getErrCode())
    {
        ...
        sprintf(mesgx,"Command: %x Response: %x Error: %x\n",
            cmd.getCode(),rsp.getCode(),rsp.getULong(2));
        lf.log(mesgx);
        com_error_counter=0;
        //releaseSerialMutex();
        return(2);
...
//    code=((unsigned short)code_hi)*256 + (unsigned short)code_lo;
    ...
    if (length>256)
    {
        sprintf(mesgx,"checkCameraMessages: Length Too Long:\n  Response: %x Rsp Length: %d\n",
            rsp.getCode(),length);
        lf.log(mesgx);
        #ifndef USEASYNSERIAL
        serial_port->clearPipe();
...
int pco::setPcoImageReadParams(int function)
{
    ...
    if (function ==pco_play_slow || function ==pco_play_stop|| function==pco_setallparams)
    {
      ...
      if (doSerialCommand(cmd,rsp,0,0)==0)
        {
          ...
          {
                //if is_allow_kill_acquire is true, then recording was started, and now its done.
              // if is_allow_kill_acquire is false, then there was no recording, so leave ADAcquire alone in case
              // someone just set it to 1, requestig a new recording. this aviods race condition.
...
int pco::getPcoImageReadParams(void)
{
 ...
 char msgx[256];
    ...
    for (k=1;k<=4;k++)
    {
          ...
          if (doSerialCommand(cmd,rsp,0,0)==0)
          {
              sprintf(msgx,"Seg 0x%x ResHV %d,%d BinXY (%d , %d) ROIXY0 (%d,%d) ROI1XY (%d,%d) \n",
                  rsp.getUShort(2),
                  rsp.getUShort(4),
                  rsp.getUShort(6),
                  rsp.getUShort(8),
...
int pco::setPcoCameraLinkParams(int function)
{
 ...
 char msgx[256];
  ...
  if (function ==pco_camlink_pixfreq ||
      function ==pco_cclines ||
      function ==pco_camlink_pixperclk ||
      function ==pco_camlink_cont|| function==pco_setallparams)
  {
...
//      getIntParam(pco_camera_ram_page_size) * getIntParam(pco_camera_ram_npages));
...
//Set Timebase
 ...
 if (function ==(pco_exp_timebase) ||
     function ==(pco_dly_timebase)|| function==pco_setallparams)
 {
...
// set exptime delay time
 if (
    (getIntParam(pco_is_frame_rate_mode) == 0 ) &&

     (function == (pco_delay_time) || function == (ADAcquireTime)|| function==pco_setallparams)
...
// set exptime delay time
 if (
        (getIntParam(pco_is_frame_rate_mode) == 1 ) &&
    (function ==(pco_set_frame_rate) ||
     ...
     )
 {
    ...
    if (function ==(ADAcquirePeriod))
    {
        ...
        if (getDoubleParam(ADAcquirePeriod)>0.0)
            setDoubleParam(pco_set_frame_rate,1.0/getDoubleParam(ADAcquirePeriod));
    ...
    rsp.setErrCode(0x18D2);
     ...
     if (doSerialCommand(cmd,rsp,0,0)==0)
     {
         ...
         setDoubleParam(ADAcquireTime,((double)ex) / 1e9);
             ...
             {
                    setDoubleParam(ADAcquirePeriod,1.0/getDoubleParam(pco_set_frame_rate));
             }
...
int pco::doHighLevelParams(int function)
{
    ...
    if (function ==(ADAcquire))
    {
        ...
        if (getIntParam(ADAcquire)==1)
        {
            ...
            //here we are getting 2 mutexes--- we want to make sure grabber loop is not doing somtehing important..
            //sio wegrab is nutex
            ...
            //getPcoRecordingParams();
            //Sleep(100);
            // once we start recording- allow driver to turn off ADAcquire when recording is done. 
            // this prevents threading race condition problem where getstatus params from camera always tirns off ADACquire
            // once every vew seconds, that may happen same time as user hits ADAcquire. 
            
            //for dimax- we acquire into RAM. then the camera stops recording.
            // we wnat to turn off ADACquire when camera stops recording, when mem is full
            //for dimax only
        ...
        if (getIntParam(ADAcquire)==0)
        {
            ...
            //we get grabber mutex to make sure it is not doing anything in grabber loop.
    ...
    if (function ==(pco_reset_memory) || function ==(ADNumImages)|| function==pco_setallparams)
    {
...
void pco::dumpCameraMemory(void)
{
    ...
    //lf.log(stxx);
    //max_fps=((float)max_data_rate) /((float)(getIntParam(ADSizeX)*getIntParam(ADSizeY)*2));
    //if (max_fps>0.0)
...
//        waitms = getIntParam(pco_dump_waitms);
    ...
    //we will grab images on THIS tgread. The grabber thread will block and do nothing.
    grabGrabberMutex();
    
    //read one image at a time.
    for (k=getIntParam(pco_dump_counter);k<=edx;k++)
    {
        ...
        // checks to see if pcoarray counter increments... it wont, because we killed the 
        // grabber thread... 
        waitcounter = 0;
        ...
        // this has to run ONCE- bacause pro array counter will not increment until grabber gets a frame
        // because we stopped the grabber thread, no frame is ther.
        //so we call oneLoopImage at least once.
        while (last_val == getIntParam(pco_array_counter)  &&
            waitcounter<200)
        {
        
            // run the grabber code once to get the frame
        // it MAY not get frame if it did not arrive from camera yet. so we have to check.
            oneLoopImage();
            ...
            if (getIntParam(pco_cancel_dump)==1)
            {
              lf.log("Dump can celled");
              k=100000000;//force break from for loop
...
void pco::resetDimaxMemory(void)
    {
        ...
        double nimgsfudge=((double)getIntParam(pco_dimax_nimgs_fudge)+(double)getIntParam(ADNumImages));
        if (nimgsfudge<3.0)
            nimgsfudge=3.0;
        ...
        if (getIntParam(pco_cdi_mode))
            req_pages=req_pages*2;
...
etb=1e-3;
  ...
  if (doSerialCommand(cmd,rsp,0,0)==0)
  {
      ...
      if (getIntParam(pco_exp_timebase)==1)
          etb=1e-6;
      ...
      if (getIntParam(pco_dly_timebase)==1)
          dtb=1e-6;
...
int pco::getPcoGeneralParams(void)
{
 ...
 char msgx[256];
  ...
  rsp.setExpCode(0x0190);
    ...
    if ((val1&0xff00) == 0x0100)
        setStringParam(ADModel,"PCO 1200 Camera");

    if ((val1&0xff00) == 0x0200)
        setStringParam(ADModel,"PCO 13k-4k Camera");

    if ((val1&0xff00) == 0x0800)
        setStringParam(ADModel,"PCO 13k14k Camera");

    if ((val1&0xff00) == 0x0900)
        setStringParam(ADModel,"PCO Pixelfly Camera");
    ...
    if ((val1&0xff00) == 0x1200)
        setStringParam(ADModel,"PCO Sensicam Camera");
...
//Reset Settings to Default
 ...
 if (function ==(pco_reset_default_settings)  || function==pco_setallparams)
 {
...
//Initiate Selftest Procedure
 ...
 if (function == pco_global_shutter && getIntParam(pco_which_camera)==pco_edge)
 {
    ...
    if (p0!=p0new)
    {
        ...
        sprintf(mesgx,"pco::setPcoGeneralParams - mcf/ccf file name %s",getStringParam(cor_ccf_filename));
        lf.log(mesgx);
...
//Set ROI
    ...
    if (
        function ==ADSizeY||
        function ==ADSizeX || function==pco_setallparams)
    {
        double x0,y0,x1,y1;
        ...
        if (getIntParam(pco_which_camera)==pco_edge)
        {
            if (getIntParam(ADSizeY)<600)
                setIntegerParam(ADSizeY,600);


            if (getIntParam(pco_edge_fastscan)==1)
            {
            #ifdef _LIMITIMGSIZE
                if (getIntParam(ADSizeX) > 1800)
                    setIntegerParam(ADSizeX,1800);


                if (getIntParam(ADSizeY) > 1800)
                    setIntegerParam(ADSizeY,1800);
            #endif
            }
        ...
        // in dimax pco_desc.wRoiHorStepsDESC is 24. We use 48 , so we have divisable by 16 too, for grabber.
        ...
        y0=1.0+ (floor(((y0-1.0)/pco_desc.wRoiVertStepsDESC)+0.5) * pco_desc.wRoiVertStepsDESC);
        y1=(floor(((y1)/pco_desc.wRoiVertStepsDESC)+0.5) * pco_desc.wRoiVertStepsDESC);
...
//Set Binn
    ...
    if ((function ==ADBinX ||
        function ==ADBinY || function==pco_setallparams) &&
        (getIntParam(pco_which_camera)==pco_dimax)
        )
    {
