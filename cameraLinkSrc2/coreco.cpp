...
void coreco::setNumBuffers(int b)
{
num_buffers=b;
}
...
void coreco::setDoubleWidth(int isdw)
{
    is_double_width=isdw;

}
bool coreco::isMissedFrame(void)
{
...
long coreco::getTotalMissedFrames(void)
{
    return(missed_frames);
}

long coreco::getRecentMissedFrames(void)
{
...
bool coreco::getFrame(void *copy_memory,unsigned int *coreco_timestamp)
{
    ...
    if (isFrameAvailable())
    {
        // getIndex returns last buffer camera dumped to. not the next one to read from...
        ...
        memcpy(copy_memory,image_address,(sensor_height*sensor_width*sizeof(unsigned short)));
        frames_to_cpu++;
        sap_result = m_Buffers->ReleaseAddress (grab_index, (void *) image_address);
        if (sap_result == false)
                    throw ccd_exception("release Address failed");

        if (sap_result == false)
            throw ccd_exception("SAP getAddress failed");
...
//
bool coreco::getFrame(void *copy_memory,unsigned int *coreco_timestamp,int nbytes)
{
//    bool sap_result;
//    int grab_index;
//    unsigned short  *image_address;
...
int coreco::getWidth(void)
{

    return(sensor_width);
}

int coreco::getHeight(void)
{
...
void coreco::setConfigFileName(char *name)
{
...
bool coreco::CreateObjects()
{
    ...
    if (m_Buffers && !*m_Buffers && !m_Buffers->Create())
   {
      char mm[128];
       DestroyObjects();
        sprintf(mm,"coreco::CreateObjects could not create SapBuffers %i %i ",sensor_height, sensor_width);
