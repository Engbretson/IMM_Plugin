...
static int (*clSerialRead)(void* serialRef, char* buffer,unsigned long* bufferSize,unsigned long serialTimeout);
static int (*clSerialWrite)(void* serialRef, char* buffer,unsigned long* bufferSize,unsigned long SerialTimeout);
static void (*clSerialClose)(void* serialRef);

static int  (*clSetParity)    (void* serialRef,unsigned int parityOn);
static int  (*clGetManufacturerInfo)    (char* ManufacturerName,unsigned int* bufferSize,unsigned int* version);
static int  (*clGetNumSerialPorts )     (unsigned int* numSerialPorts);
static int  (*clGetSerialPortIdentifier)(unsigned int serialIndex,char* portID,unsigned int* bufferSize);
...
static int  (*clGetErrorText)            (int errorCode,char *errorText,unsigned int* errorTextSize);
...
void siswSerialPort::readDll(void)
{
    ...
    clSerialInit = (int (__cdecl*)(unsigned long serialIndex, void** serialRefPtr))GetProcAddress (hMod, "clSerialInit");
    clSerialRead = (int (__cdecl*)(void* serialRef, char* buffer,unsigned long* bufferSize,unsigned long serialTimeout))GetProcAddress (hMod, "clSerialRead");
    clSerialWrite = (int (__cdecl*)(void* serialRef, char* buffer,unsigned long* bufferSize,unsigned long SerialTimeout))GetProcAddress (hMod, "clSerialWrite");
    clSerialClose = (void (__cdecl*)(void* serialRef))GetProcAddress (hMod, "clSerialClose");
    
    clSetParity = (int (__cdecl*)(void* serialRef,unsigned int parityOn)) GetProcAddress (hMod, "clSetParity");
    clGetManufacturerInfo = (int (__cdecl*) (char* ManufacturerName,unsigned int* bufferSize,unsigned int* version) )GetProcAddress (hMod,  "clGetManufacturerInfo");
    clGetNumSerialPorts = (int (__cdecl*) (unsigned int* numSerialPorts)) GetProcAddress (hMod,  "clGetNumSerialPorts");
    clGetSerialPortIdentifier = (int (__cdecl*) (unsigned int serialIndex,char* portID,unsigned int* bufferSize) )GetProcAddress (hMod,  "clGetSerialPortIdentifier");
    clGetNumBytesAvail = (int (__cdecl*) (void* serialRef,unsigned int* bufferSize) )GetProcAddress (hMod,  "clGetNumBytesAvail");
    clFlushPort = (int (__cdecl*) (void* serialRef) )GetProcAddress (hMod,  "clFlushPort");
    clGetSupportedBaudRates = (int (__cdecl*) (void* serialRef,unsigned int* baudRates)) GetProcAddress (hMod,  "clGetSupportedBaudRates");
    clSetBaudRate = (int (__cdecl*) (void* serialRef,unsigned int baudRate)) GetProcAddress (hMod,  "clSetBaudRate");
    clGetErrorText = (int (__cdecl*)(int errorCode,char *errorText,unsigned int* errorTextSize) )GetProcAddress (hMod,  "clGetErrorText");
...
void siswSerialPort::read(unsigned char *buffer, int length)
{
    ...
    if (is_open)
    {
        ...
        stat=clSerialRead(serial_ref, (char*)buffer, (unsigned long* )&length, 200);
        readcnt=0;
