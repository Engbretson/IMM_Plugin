...
asynStatus pcoEdgePlugin::openFile(const char *fileName, NDFileOpenMode_t openMode, NDArray *pArray)
{
    ...
    if (getIntParam(is_enable)>0)
    {
        ...
        if (mypool)
        {
            ...
            if (getIntParam(edge_nd_datasize)>0)
            {
                ...
                ((getIntParam(edge_max_ndmemory)- getIntParam(edge_alloc_ndmemory))/getIntParam(edge_nd_datasize)  )     );
            }
        ...
        if (getIntParam(is_decompress)==1)
        {
            ...
            {
                printf("ERROR- Decompression shorts in= %f,   img size= %d",shorts_in,xs);
            ...
            for (rows = 0 ;rows<ys; rows++)
            {
                ...
                while(k<len_in)
                {
                    ...
                    //we assume we are using the 16-12 lut in edge camera. the table is defined
                    //as y=x for x<=256
        ...
        else
        {
            // if no 12 to 16 bit extraction, then copy raw to temp img
                ...
                memcpy((void*)temp_image,(void*)pArray->pData,xs*ys*sizeof(short));
        }    
...
extern "C" int drvpcoEdgePluginConfigure(const char *portName, int queueSize, int blockingCallbacks,
                                     const char *NDArrayPort, int NDArrayAddr,
                                     int priority, int stackSize)
{
    new pcoEdgePlugin(portName, queueSize, blockingCallbacks, NDArrayPort, NDArrayAddr,
                     priority, stackSize);
    return(asynSuccess);
}

/* The constructor for this class */
pcoEdgePlugin::pcoEdgePlugin(const char *portName, int queueSize, int blockingCallbacks,
                           const char *NDArrayPort, int NDArrayAddr,
                           int priority, int stackSize)
    : NDPluginFile(portName, queueSize, blockingCallbacks,
                  NDArrayPort, NDArrayAddr,
           1, NUM_pcoEdgePlugin_PARAMS, 200000, -1, asynGenericPointerMask, asynGenericPointerMask,
...
void pcoEdgePlugin::sort_lines_A(int width,int height,void *adr_out,void* adr_in)
{
...
void pcoEdgePlugin::sort_lines_B(int width,int height,void *adr_out,void* adr_in)
{
  unsigned short *line_1;
  unsigned short *line_n;
  unsigned short *line_in;
...
void pcoEdgePlugin::sort_lines_C(int width,int height,void *adr_out,void* adr_in)
{
  unsigned short *line_1;
  unsigned short *line_n;
  unsigned short *line_in;
...
void pcoEdgePlugin::sort_lines_D(int width,int height,void *adr_out,void* adr_in)
{
  unsigned short *line_1;
  unsigned short *line_n;
  unsigned short *line_in;
